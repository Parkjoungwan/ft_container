...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/20 13:40:40 by joupark           #+#    #+#             */
/*   Updated: 2022/06/20 14:35:52 by joupark          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	//RAII 패턴: 함수 내부에서 예외가 발생하면서 소멸자가 호출하는 코드에 도달하지 못할 수 있다.
	//따라서 자원을 미리 스택에 할당하는 식으로 관리해, 해당 함수의 스택에 정의되어 있는 
	//모든 객체들이 빠짐없이 소멸자가 호출될 수 있게 만드는 것이다.(stack unwinding).
	//vector_base: RAII패턴을 위한 클래스, vector의 allocator는 여기서 할당 받게된다.
	...
	//vector_iterator: vector용 iterator, iterator_traits 응용
	...
	class __vector_iterator
	{
		...
		public:
			...
			__vector_iterator(
				...
				typename enable_if<__is_random_access_iterator<_U>::value>::type* = 0
			)  // can copy when the category is random_access
	...
	class vector : private __vector_base<_T, _Allocator> 
	{
		...
		public:
			...
			// modifiers
			template <typename _InputIterator>
			void assign(_InputIterator first,
						typename enable_if<__is_input_iterator<_InputIterator>::value &&
						!__is_forward_iterator<_InputIterator>::value,
						_InputIterator>::type last
						);
			...
			template <typename _InputIterator>
			void insert( iterator position, _InputIterator first,
					typename enable_if<__is_input_iterator<_InputIterator>::value &&
					!__is_forward_iterator<_InputIterator>::value,
					_InputIterator>::type last
					);
	...
	template <typename _T, typename _Allocator>
	template <typename _ForwardIterator>
	vector<_T, _Allocator>::vector( _ForwardIterator first,
			typename enable_if<__is_forward_iterator<_ForwardIterator>::value,
			_ForwardIterator>::type last,
			const allocator_type& Alloc
			)
	...
	template <typename _T, typename _Allocator>
	template <typename _InputIterator>
	void vector<_T, _Allocator>::assign(_InputIterator first,
		typename enable_if<__is_input_iterator<_InputIterator>::value &&
			!__is_forward_iterator<_InputIterator>::value,
		_InputIterator>::type last)
	{
