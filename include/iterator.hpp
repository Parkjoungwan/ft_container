...
namespace ft
{
	...
	//iterator_Base
	//value_type: 자료형
	//difference_type: iterator 사이의 거리
	//pointer: 자료형 포인터
	//reference: 참조 자료형
	//iterator_category: iterator 카테고리 태그
	...
	//iterator_traits_typedefs: 특정 Iterator를 기본형으로 만들기 위한 구조체
	...
	//is_iterator: 매개변수로 받아온 iter가 iterator가 맞는지 확인
	template <typename _Iter,
	...
	//reverse_iterator: iterator를 상속받아와 만든다. 증간연산자를 이용한
	//탐색 움직임이 다르게 정의된다.
	template <typename _Iter>
	...
	//inline키워드: 특정함수를 컴파일 단계에서 특정 코드로 아예 대체해버림
	//distance: 특정 iterator간의 거리를 구하는 함수
